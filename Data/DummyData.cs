using LMSStudent.Models;
using Microsoft.AspNetCore.Builder;
using Microsoft.Extensions.DependencyInjection;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace LMSStudent.Data
{
    public class DummyData
    {
        public static void Initialize (IApplicationBuilder app)
        {
            using (var serviceScope = app.ApplicationServices.GetService<IServiceScopeFactory>().CreateScope())
            {
                var context = serviceScope.ServiceProvider.GetService<LMSStudentDBContext>();
                context.Database.EnsureCreated();

                if (context.Topics.Count() == 0)
                {
                    var topics = new List<Topic>()
                    {
                    new Topic {Name = "C#"},
                    new Topic {Name = "ASP.Net"},
                    new Topic {Name = "Java"},
                    new Topic {Name = "JavaScript"},
                    new Topic {Name = "Angular"}
                    };

                    context.Topics.AddRange(topics);
                    context.SaveChanges();
                }

                if (context.Resources.Count() == 0)
                {
                    var resources = new List<Resource>()
                    {
                    new Resource {Name = "Exercise 1", Description = "Exercise 1 desc", File = "word", TopicId = 1 },
                    new Resource {Name = "Exercise 2", Description = "Exercise 1 desc", File = "excel", TopicId = 2 },
                    new Resource {Name = "Exercise 3", Description = "Exercise 1 desc", File = "pdf", TopicId = 2 },
                    new Resource {Name = "TeachingMaterial 1", Description = "TeachingMaterial 1 desc", File = "word", TopicId = 3 },
                    new Resource {Name = "TeachingMaterial 2", Description = "TeachingMaterial 1 desc", File = "excel", TopicId = 3 }
                    };

                    context.Resources.AddRange(resources);
                    context.SaveChanges();
                }

                if (context.Exercises.Count() == 0)
                {
                    var exercises = new List<Exercise>()
                    {
                    new Exercise {AvailableTime = 2, ResourceId = 5},
                    new Exercise {AvailableTime = 2, ResourceId = 4},
                    new Exercise {AvailableTime = 2, ResourceId = 3}
                    };

                    context.Exercises.AddRange(exercises);
                    context.SaveChanges();
                }

                if (context.TeachingMaterials.Count() == 0)
                {
                    var teachingMaterials = new List<TeachingMaterial>()
                    {
                    new TeachingMaterial {Type = "Word", Link = "www.itacademy.com", ResourceId = 2},
                    new TeachingMaterial {Type = "Excel", Link = "www.itacademy2.com", ResourceId = 1}
                    };

                    context.TeachingMaterials.AddRange(teachingMaterials);
                    context.SaveChanges();
                }
            }
        }              
    }
}
